...
class JuraClient {
  ...
  async brew(product, overrides = {}, opts = {}) {
    ...
    if (opts.retry && !isBrewAccept(reply)) {
      // Energy-safe wake-up: the first @TP: only woke the machine; it
      // needs a moment to actually finish waking before it can process
      // a second command -- retrying instantly got the exact same
      // not-a-@tp-reply reply back both times on a real E8, confirmed
      // live. A short pause before resending gives that wake-up time
