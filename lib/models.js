 ...
 * catalogue are bundled here. Only two have been live-verified against
 * real hardware so far -- the E8 (EF538, article 15336 / hwId EF538M)
 * and the ENA 4 (EF1013) -- see README.md "What's verified" for the
 * current list. Every other profile's product/recipe data is extracted
 * the same way (see lib/profiles/README.md) but untested against a
 * physical machine.
 *
...
const MODELS = [
  ...
  },
  {
    ...
    // Live-verified 2026-08-10 by an external tester (Dijker, GitHub
    // issue #1) on a real ENA 4 (EB), article 15501 -- pairing, brewing
    // (coffee + espresso), and all five alarms confirmed. EB/SA/UKA/CNA/
    // etc. share this EF code per jura_connect's own catalogue (regional
    // SKUs of the same protocol family, not distinct hardware), so the
    // verification covers the family, not just the EA variant in the label.
    verified: true,
  },
  {
